...
@Service
public class CategoryIconUploadService {
    ...
    @Value("${minio.category-icon-prefix}")
    private String iconPrefix;

    @Value("${minio.presigned-expiry-minutes}")
    private int expiryMinutes;

    public CategoryIconUploadService(S3Presigner s3Presigner) {
        this.s3Presigner = s3Presigner;
    }

    public PresignedUploadUrlResponse createUploadUrl(String categoryId, String fileName, String contentType) {
        String safeCategoryId = sanitizePathSegment(categoryId);
    ...
    /**
     * Returns a stable, publicly-readable URL for a category icon. Category
     * icons live under a prefix configured with anonymous read access, so no
    ...
    private String sanitizePathSegment(String value) {
        ...
        String cleaned = value
                ...
                .replaceAll("-{2,}", "-")
                .replaceAll("(^-|-$)", "");
        return cleaned.isBlank() ? "unknown-category" : cleaned;
    }
}
