package com.redstore.cart.config;
...
public class NatsConfig {
    ...
    @Value("${cart.nats.stream}")
    private String stream;

    @Value("${cart.nats.durable}")
    private String durable;

    @Value("${cart.nats.filter-subject}")
    ...
    public JetStreamSubscription productEventsSubscription(JetStream js) throws Exception {
        ...
        for (int i = 1; i <= attempts; i++) {
            try {
                log.info("cart-service: subscribing (attempt {}) stream={} durable={} subject={}",
                ...
                log.warn("cart-service: subscribe attempt {} failed ({}); retrying", i, e.getMessage());
